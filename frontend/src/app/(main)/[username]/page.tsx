...
const page = async ({ params }: Props) => {
  ...
  return (
    <div className="w-full flex gap-6">
      <div className="grid grid-cols-1 gap-4 w-full lg:w-7/12 pb-12">
        <ProfileTopSection data={user} />

        <ProfileAbout educations={user.educations} work={user.work} />
        <ProfilePosts userid={user._id} />
      </div>
      <div className=" hidden lg:flex p-4 bg-background w-full lg:w-5/12 rounded-xl h-fit  flex-col gap-4">
        <CourseCard />
        <CourseCard />
        <CourseCard />
      </div>
